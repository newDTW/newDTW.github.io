...
function boxf(left, top, right, bottom){
	left = left || 0;
	top = top || 0;
	right = right || 680;
	bottom = bottom || 680;
...
function limit(val, min_val, max_val){
	return parseInt(Math.max(min_val, Math.min(val, max_val)));
}
...
function palette(data1, data2, data3, data4, data5){}
...
function picload(img_name, func){
	if(!func){
		throw "関数が無いとうまく動かない!";
	}
	img = new Image();
...
function screen(id, display_width, display_height, init_mode, pos_x, pos_y){
	if(id == 0){
		display_width = 340;
		display_height = 340;
	}
...
function sdim(length1, length2, length3, length4){
	return dim(length1, length2, length3, length4);
}
function stick(data0){
	return 0;
}
function title_(window_name){
